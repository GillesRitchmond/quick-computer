...
<body class="body">
    ...
    $query = "SELECT count(personne.id_person) AS qty_personne FROM personne, groupe, entreprise, users, program WHERE users.code_entreprise = '$code_entreprise' AND
             entreprise.code_entreprise = '$code_entreprise' AND program.code_user = users.code_user AND
             personne.id_group = groupe.id_group AND program.id_program = personne.id_program AND nom like '%$nom%' AND prenom LIKE '%$prenom%'";
    $result = $conn->query($query);
    ...
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            echo '<li class="data-title">Qty of person added by the team:</li>
                ...
                <li class="data-hint">Make report to make a better management of persons</li>';
        }
    }
    echo '</ul>
                    </div>
